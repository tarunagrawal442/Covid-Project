...
data1<- data %>% select(date,continent,location,population,total_cases) %>% filter(!is.na(continent)) %>% mutate(tc_percentage=(total_cases/population)*100, na.rm=TRUE) %>% arrange(location)

# Grouping by location which is country
data2<- data1 %>% group_by(location) %>% summarise(popluation=mean(population,na.rm=TRUE),total_case=max(total_cases, na.rm = FALSE),tc_percentage=max(tc_percentage,na.rm = FALSE)) %>% arrange(tc_percentage)
...
death_data_cases<- data %>% filter(!is.na(continent)) %>% select(date,continent,location,population,total_cases,total_deaths) %>% group_by(location) %>% summarise(case=max(total_cases,na.rm = TRUE), deaths=max(total_deaths,na.rm = TRUE))%>%mutate(death_percentage=(deaths/case)*100) %>% arrange(location)
...
vaccination_total_percent<-vaccination_total %>% mutate(per_vaccinatatd=total_vaccinations/population*100)
